...
class LabeledTextInput:
    def __init__(self, label_text, on_lose_focus=None, readonly=False):
        self.box = toga.Box(style=style_box_row)
        self.label = toga.Label(label_text, style=style_label_input)
        self.text_input = toga.TextInput(style=style_input, on_lose_focus=on_lose_focus, readonly=readonly)
        self.box.add(self.label)
    ...
    class LabeledMultilineTextInput:
        def __init__(self, label_text, on_lose_focus=None, readonly=False):
            self.box = toga.Box(style=style_box_row)
            self.label = toga.Label(label_text, style=style_label_input)
            self.text_input = toga.MultilineTextInput(style=style_input, on_lose_focus=on_lose_focus, readonly=readonly)
            self.box.add(self.label)
            self.box.add(self.text_input)
    ...
    class LabeledSwitch:
        def __init__(self, label_text, on_change=None):
            self.box = toga.Box(style=style_box_row)
            self.label = toga.Label(label_text, style=style_label_input)
            self.switch = toga.Switch(style=style_switch, on_change=on_change)
            self.box.add(self.label)
